...
class GranolaMCPServer:
    ...
    def _parse_date_query(self, query: str) -> Optional[Tuple[datetime, datetime]]:
        ...
        # Check if query contains both a day name and "last week" or "last" + day pattern
        has_last_week = "last week" in query_lower
        has_last = "last" in query_lower
        
        if day_match and (has_last_week or (has_last and day_match)):
            days_since_monday = now.weekday()
            last_monday = (now - timedelta(days=days_since_monday + 7)).replace(hour=0, minute=0, second=0, microsecond=0)
